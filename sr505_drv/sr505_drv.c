 ...
 * sr505_drv.c
 *
 * HC-SR505 PIR driver for RK3566 GPIO2_C6.
 *
 * GPIO2_C6 = 2 * 32 + 2 * 8 + 6 = 86
 *
 * 说明：
 * 1. 不依赖设备树，驱动中直接固定 GPIO86。
                          ...
                          size_t count, loff_t *ppos)
{
    ...
    struct sr505_device *dev = container_of(mdev, struct sr505_device, miscdev);
    unsigned char state;
    int ret;
    ...
    if (!atomic_read(&dev->pending)) {
        ...
        ret = wait_event_interruptible(dev->wait, atomic_read(&dev->pending));
        if (ret)
            return ret;
    }
...
static int __init sr505_init(void)
{
    ...
    pr_info(DRIVER_NAME ": loading, GPIO=%d GPIO2_C6\n", dev->gpio);

    if (!gpio_is_valid(dev->gpio)) {
        pr_err(DRIVER_NAME ": invalid GPIO %d\n", dev->gpio);
        return -EINVAL;
    }

    ret = gpio_request(dev->gpio, DRIVER_NAME "_gpio");
    if (ret) {
        pr_err(DRIVER_NAME ": gpio_request failed: %d\n", ret);
        return ret;
    }

    ret = gpio_direction_input(dev->gpio);
    if (ret) {
        pr_err(DRIVER_NAME ": gpio_direction_input failed: %d\n", ret);
        goto err_gpio;
    }

    /*
     * 读取初始状态。
     * pending 设置为 1，是为了让应用启动后能立即读到当前 OUT 初值。
     */
    ...
    if (dev->irq < 0) {
        ret = dev->irq;
        pr_err(DRIVER_NAME ": gpio_to_irq failed: %d\n", ret);
        goto err_gpio;
    }
     ...
     */
    ret = request_threaded_irq(dev->irq, NULL, sr505_irq_thread,
                               IRQF_TRIGGER_RISING |
                               IRQF_TRIGGER_FALLING |
                               IRQF_ONESHOT,
                               DRIVER_NAME, dev);
    if (ret) {
        pr_err(DRIVER_NAME ": request_threaded_irq failed: %d\n", ret);
        goto err_gpio;
    }
    ...
    pr_info(DRIVER_NAME ": loaded, /dev/%s, GPIO%d -> IRQ%d, initial OUT=%d\n",
            DEVICE_NAME, dev->gpio, dev->irq, state);
...
MODULE_DESCRIPTION("HC-SR505 PIR driver for RK3566 GPIO2_C6");
MODULE_VERSION("1.0");
